...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name_fr) || empty($name_ar) || empty($lat) || empty($lng)) {
        ...
        if(empty($error)) {
            $stmt = $conn->prepare("INSERT INTO locations (name_fr, name_ar, type, latitude, longitude, description_fr, description_ar, working_hours_fr, working_hours_ar, contact_info, image, service_link) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssddssssssss", $name_fr, $name_ar, $type, $lat, $lng, $desc_fr, $desc_ar, $hours_fr, $hours_ar, $contact, $image_name, $link);
...
<!-- Include Leaflet to help pick coordinates -->
                    ...
                    <form action="" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row mb-4">
                            <div class="col-md-12 mb-2">
                                <label class="form-label fw-bold">Pick Location Coordinates on Map *</label>
                                <div id="map-picker"></div>
                                <small class="text-muted">Click on the map to drop a pin and set the coordinates.</small>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label text-muted">Latitude</label>
                                <input type="text" class="form-control bg-light" id="lat" name="latitude" required readonly>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label text-muted">Longitude</label>
                                <input type="text" class="form-control bg-light" id="lng" name="longitude" required readonly>
                            </div>
                        </div>
                        ...
                        <div class="row mb-4">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" name="working_hours_fr" placeholder="e.g. Lun-Ven: 08:30 - 16:30">
                            </div>
                            <div class="col-md-6" dir="rtl">
                                <label class="form-label fw-bold">أوقات العمل (بالعربية)</label>
                                <input type="text" class="form-control" name="working_hours_ar" placeholder="مثال: الإثنين-الجمعة: 08:30 - 16:30">
                            </div>
                        </div>

                        <div class="mb-4">
                            <label class="form-label fw-bold">Service Redirect Link (Optional)</label>
                            <input type="text" class="form-control" name="service_link" placeholder="../citizen/service.php">
                            <small class="text-muted">URL to redirect users to when they click 'Visit Service' in the map popup.</small>
                        </div>

                        <button type="submit" class="btn btn-primary btn-lg px-5">Save Location</button>
                    </form>
                </div>
            </div>
        </main>
...
<script>
    // Map Picker Logic
    const map = L.map('map-picker').setView([32.44425, -6.96037], 15);
